...
use crate::model::{annotation_label, hex_color_to_rgb, Annotation, SidebarTreeRow};
...
use crate::state::{
	active_file_key, active_viewport_from_snapshot, host_api, host_snapshot, log_message,
...
fn focus_sidebar_row(row_id: &str) -> Result<(), String> {
	...
	let annotation_target = {
		...
		if let Some((set_id, _, _)) = target.as_ref() {
			state.selected_set_by_file.insert(active_path, set_id.clone());
		}
		target
	};
