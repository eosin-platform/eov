...
pub(crate) fn build_host_api(
    ...
    HostApiVTable {
        ...
        toggle_series_bar: ffi_toggle_series_bar,
        cycle_series_selection: ffi_cycle_series_selection,
        activate_selected_series_entry: ffi_activate_selected_series_entry,
...
pub(crate) fn toggle_series_bar() -> Result<(), String> {
    run_on_ui_thread(move |runtime| {
        let ui = runtime
            .ui_weak
            .upgrade()
            .ok_or_else(|| "application window is no longer available".to_string())?;
        crate::callbacks::toggle_series_bar_requested(
            &ui,
            &runtime.state,
            &runtime.tile_cache,
            &runtime.render_timer,
...
pub(crate) fn cycle_series_selection(offset: i32) -> Result<(), String> {
    run_on_ui_thread(move |runtime| {
        let ui = runtime
            .ui_weak
            .upgrade()
        ...
        {
            let mut state = runtime.state.write();
            state.cycle_series_selection(offset);
        }
        {
            let state = runtime.state.read();
            crate::update_tabs(&ui, &state);
...
pub(crate) fn activate_selected_series_entry() -> Result<(), String> {
    run_on_ui_thread(move |runtime| {
        let ui = runtime
            .ui_weak
            .upgrade()
            .ok_or_else(|| "application window is no longer available".to_string())?;
        let Some(path) = runtime.state.read().selected_series_entry_path() else {
            return Ok(());
        };
        crate::callbacks::activate_series_entry_path(
            &ui,
            &runtime.state,
            &runtime.tile_cache,
            &runtime.render_timer,
...
fn build_viewport_overlay_layer_factory(
    ...
    let signature = ViewportOverlayFactorySignature {
        visible_panes: snapshots
            .iter()
            .map(|snapshot| snapshot.pane_index as i32)
            .collect(),
    ...
    for (index, request) in sorted_requests.iter().enumerate() {
        ...
        let overlay_props = (request.vtable.get_viewport_overlay_properties)(
            to_viewport_snapshot_ffi(binding_snapshot.clone()),
        );
        ...
        for (child_property, root_property) in STANDARD_BINDINGS {
            if property_types
                .iter()
                .any(|(candidate, _)| candidate == child_property)
            {
                children.push_str(&format!(
                    "        {}: root.{};\n",
    ...
    let definition = result
        .component("PluginViewportOverlayLayer")
        .ok_or_else(|| {
            format!(
                "viewport {} overlay composite component missing",
                layer.label()
            )
        })?;
...
fn apply_viewport_overlay_properties(
    ...
    let standard_values = [
        (
            "pane_index",
            slint_interpreter::Value::Number(snapshot.pane_index as f64),
        ),
        (
            "viewport_center_x",
            slint_interpreter::Value::Number(snapshot.center_x),
        ),
        (
            "viewport_center_y",
            slint_interpreter::Value::Number(snapshot.center_y),
        ),
        (
            "viewport_zoom",
            slint_interpreter::Value::Number(snapshot.zoom),
        ),
        (
            "viewport_width",
            slint_interpreter::Value::Number(snapshot.width),
        ),
        (
            "viewport_height",
            slint_interpreter::Value::Number(snapshot.height),
        ),
        (
            "image_width",
            slint_interpreter::Value::Number(snapshot.image_width),
        ),
        (
            "image_height",
            slint_interpreter::Value::Number(snapshot.image_height),
        ),
        (
            "bounds_left",
            slint_interpreter::Value::Number(snapshot.bounds_left),
        ),
        (
            "bounds_top",
            slint_interpreter::Value::Number(snapshot.bounds_top),
        ),
        (
            "bounds_right",
            slint_interpreter::Value::Number(snapshot.bounds_right),
        ),
        (
            "bounds_bottom",
            slint_interpreter::Value::Number(snapshot.bounds_bottom),
        ),
    ];
    ...
    for contribution in contributions {
            ...
            .into_iter()
        {
            ...
            instance
                .set_property(&binding.root_property, value)
                .map_err(|err| {
                    format!(
...
fn ensure_viewport_overlay_layer(
    ...
    if needs_rebuild {
        let (factory, layer_state) =
            build_viewport_overlay_layer_factory(layer, requests, snapshots)?;
        viewport_overlay_instances(layer).with(|slot| slot.borrow_mut().clear());
        viewport_overlay_state(layer).with(|slot| {
...
extern "C" fn ffi_toggle_series_bar(context: u64) -> RResult<(), RString> {
    if context_state(context).is_err() {
        return RResult::RErr(RString::from("invalid host API context"));
    }
    match toggle_series_bar() {
        Ok(()) => RResult::ROk(()),
        Err(err) => RResult::RErr(RString::from(err)),
    }
}

extern "C" fn ffi_cycle_series_selection(context: u64, offset: i32) -> RResult<(), RString> {
    if context_state(context).is_err() {
        return RResult::RErr(RString::from("invalid host API context"));
    }
    match cycle_series_selection(offset) {
        Ok(()) => RResult::ROk(()),
        Err(err) => RResult::RErr(RString::from(err)),
    }
}

extern "C" fn ffi_activate_selected_series_entry(context: u64) -> RResult<(), RString> {
    if context_state(context).is_err() {
        return RResult::RErr(RString::from("invalid host API context"));
    }
    match activate_selected_series_entry() {
        Ok(()) => RResult::ROk(()),
        Err(err) => RResult::RErr(RString::from(err)),
    }
}
